...
def forward(self, image, text: Optional[str] = None, attention_mask=None):
    ...
    if text is not None:
        ...
        if attention_mask is None:
            img_mask = torch.ones(projected_image_features.shape[:2], device=image.device)
        ...
        if attention_mask is None:
            attention_mask = torch.ones(combined_embeds.shape[:2], device=image.device)
    
    outputs = self.decoder(inputs_embeds=combined_embeds, attention_mask=attention_mask)
    return outputs