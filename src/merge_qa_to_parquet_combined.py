...
def create_qa_lookup(combined_qa: Dict) -> pl.DataFrame:
    ...
    num_with_1 = qa_df.filter(
        pl.col("q1").is_not_null() & pl.col("q2").is_null()
    ).height
    num_with_2 = qa_df.filter(
        pl.col("q2").is_not_null() & pl.col("q3").is_null()
    ).height
...
def merge_qa_with_parquet(parquet_path: str, qa_df: pl.DataFrame, output_path: str):
    ...
    num_with_1 = merged_df.filter(
        pl.col("q1").is_not_null() & pl.col("q2").is_null()
    ).height
    num_with_2 = merged_df.filter(
        pl.col("q2").is_not_null() & pl.col("q3").is_null()
    ).height
