...
def save_checkpoint(
    ...
    save_dict = {
        ...
        "momentum_model_state_dict": momentum_model_state_dict,
...
def load_model(config, model, momentum_model, logger, model_name="dino"):
    ...
    if config.MODEL.PRETRAINED is not None:
        all_state_dicts = torch.load(
            config.MODEL.PRETRAINED, map_location=torch.device("cpu")
        )
        # model load
        ...
        logger.info(
            f"Load Pretrained Model: {msg} for Achitecture: {config.MODEL.NAME}"
        )
        # momentum model load
        if momentum_model is not None:
            momentum_loaded_state_dict = all_state_dicts["momentum_model_state_dict"]
            momentum_new_state_dict = {
                k.replace("module.", "")
                .replace("backbone.", "")
                .replace("_orig_mod.", ""): v
                for k, v in momentum_loaded_state_dict.items()
            }
            msg = momentum_model.load_state_dict(momentum_new_state_dict, strict=False)
            logger.info(
                f"Load Pretrained Momentum Model: {msg} for Achitecture: {config.MODEL.NAME}"
            )
...
@torch.no_grad()
def _update_momentum_encoder(
    model: torch.nn.Module, momentum_model: torch.nn.Module, m: float
) -> None:
    """
